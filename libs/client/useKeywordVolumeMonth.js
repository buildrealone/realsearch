export default function useKeywordVolumeMonth({
    ...
    // korea_two_days_ago
       ...
       // const all_device_absolute_count_latest_day = ((all_labels?.includes(korea_yesterday)) || (all_labels?.includes(korea_two_days_ago))) ? all_device_absolute_counts?.[all_device_absolute_counts?.length - 1] : 0;
       // const mobile_absolute_count_latest_day = ((all_labels?.includes(korea_yesterday)) || (all_labels?.includes(korea_two_days_ago))) ? mobile_absolute_counts?.[mobile_absolute_counts?.length - 1] : 0;
       const all_device_absolute_count_latest_day = (all_labels && all_device_absolute_counts?.length > 5 && all_labels?.length === all_device_absolute_counts?.length) ? all_device_absolute_counts?.[all_device_absolute_counts?.length - 1] : 0;
       const mobile_absolute_count_latest_day = (all_labels && mobile_absolute_counts?.length > 5 && all_labels?.length === mobile_absolute_counts?.length) ? mobile_absolute_counts?.[mobile_absolute_counts?.length - 1] : 0;
